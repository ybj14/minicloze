...
The important local course codes are `mon-a1`, `mon-swadesh`, `bod-a1`, `bod-swadesh`, `tgk-a1`, `tgk-swadesh`, `tha-a1`, `tha-swadesh`, `mya-a1`, and `mya-swadesh`. User-facing aliases such as `mongolian-a1`, `tibetan-swadesh`, and `burmese-a1` are normalized in `minicloze-lib/src/langs.rs`.
...
- `scripts/build_static_web_data.py` copies corpus/vocab/explanation files into the static web directory, adds Tibetan Wylie/THL data, Thai Paiboon data, and Burmese MLCTS/Okell token data.
...
- Keep non-ASCII language text intact. Do not normalize away Tibetan tseks, Thai spelling, Cyrillic letters, or transliteration fields. Burmese explanations and tokens must preserve both `mlcts` and `okell`.
