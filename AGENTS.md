...
Tibetan learner phonetics use `scripts/tibetan_zwpy.py` (藏文拼音 / ZWPY). The launcher scripts create/reuse `.venv-tibetan` and install Botok/pyewts when needed.
...
- `scripts/build_static_web_data.py` copies corpus/vocab/explanation files into the static web directory, adds Tibetan Wylie + ZWPY (藏文拼音) data, Thai Paiboon data, Burmese MLCTS/Okell token data, and Khmer Wiktionary transliteration/transcription token data.
